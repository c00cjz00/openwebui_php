...
$j=0;
for($i=1;$i<100;$i++){
	$cmd = 'curl -X GET "'.$dify_ip.'/v1/workflows/logs?limit=100&page='.$i.'" --header "Authorization: Bearer '.$document02_key.'"';
	// Execute the command
	...
	if ($fullResponse) {
		...
		foreach ($data as $key => $arr){
			...
			if ($account==$session_id){
				...
				$url="<a href=\"document02_logs_detail.php?id={$id}\">OPEN</a>";				
				}else{
				$status="False";
				$url=$status;
				}
